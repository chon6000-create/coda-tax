...
현재: `+location.protocol);const P=e("voice-modal");if(!P){alert("오류: 음성 모달 요소를 찾을 수 없습니다.");return}if(e("voice-transcribed-text").innerText="",e("voice-result-box").style.display="none",e("voice-status-text").innerText="마이크 초기화 중...",P.style.display="flex",n.recognition)try{console.log("Calling recognition.start()..."),n.recognition.start()}catch(S){console.error("Recognition Start Error:",S),S.name!=="InvalidStateError"&&alert("마이크 시작 오류: "+S.message)}else alert("이 브라우저나 기기에서는 음성 인식을 지원하지 않습니다.")},confirmVoiceEntry:async()=>{if(!n.lastDetected){alert("인식된 내용이 없습니다.");return}if(!n.currentUser){alert("로그인 정보가 없습니다. 다시 로그인해 주세요."),s("/");return}const P=document.querySelector("#voice-result-box .btn-primary");P&&(P.disabled=!0,P.innerText="저장 중...");try{const S={...n.lastDetected};if(e("voice-modal").style.display="none",n.lastDetected=null,P&&(P.disabled=!1,P.innerText="저장하기"),n.recognition)try{n.recognition.stop()}catch{}const b=mu(yi(jr,"users",n.currentUser.uid,"records"),S),N=new Promise((F,K)=>setTimeout(()=>K(new Error("Firestore Timeout (20s)")),2e4)),O=await Promise.race([b,N]);console.log("Firestore Save Success - ID:",O.id),r("심어두기가 완료되었습니다! 🎉")}catch(S){console.error("Firestore Save Error/Timeout:",S),r("저장 응답이 지연되고 있습니다.","error")}},cancelVoiceModal:()=>{if(console.log("cancelVoiceModal clicked"),n.recognition)try{n.recognition.stop()}catch{}n.lastDetected=null,e("voice-modal").style.display="none",e("voice-transcribed-text").innerText="",e("voice-result-box").style.display="none",e("voice-status-text").innerText="듣고 있습니다..."},clearVoiceTranscript:()=>{n.recognition&&n.recognition.start()},openAddModal:()=>e("edit-modal").style.display="flex",saveManualEntry:async P=>{if(P&&P.preventDefault(),!n.currentUser){alert("로그인 정보가 없습니다."),s("/");return}const S={date:e("edit-date").value||new Date().toISOString().split("T")[0],type:e("edit-type").value,category:e("edit-category").value,amount:parseInt(e("edit-amount").value)||0,status:"준비"};try{await mu(yi(jr,"users",n.currentUser.uid,"records"),S),alert("내역이 수동으로 저장되었습니다."),e("edit-modal").style.display="none"}catch(b){console.error("Manual Save Error:",b),alert("수동 저장 실패: "+b.message)}},deleteRecord:async P=>{confirm("정말 삭제하시겠습니까?")&&await GE(bh(jr,"users",n.currentUser.uid,"records",P))},showYearlyCategorySummary:()=>{const P={};n.records.forEach(b=>{const N=b.category||"기타필요경비",O=b.label||N,F=n.categories.find(z=>z.id===N)||{},K=F.box?` [${F.box}]`:"";P[O]||(P[O]={amount:0,box:K}),P[O].amount+=Number(b.amount)||0});let S='<div style="font-size:0.9rem;">';for(const[b,N]of Object.entries(P))S+=`
                    ...
                    </div>`;S+="</div>",e("report-title").innerText="카테고리별 실적 (홈택스 기준)",e("report-content").innerHTML=S||"데이터가 없습니다.",e("report-modal").style.display="flex"},showPrevYearSummary:()=>{const S=new Date().getFullYear(),b=n.records.length,N=n.records.reduce((F,K)=>F+(Number(K.amount)||0),0);let O=`
                <div style="text-align:center; padding:1.5rem;">
                    <div style="font-size:0.85rem; color:var(--text-muted); margin-bottom:0.5rem;">${S}년 누적 실적</div>
