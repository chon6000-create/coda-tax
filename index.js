 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class $e{constructor(e,t){if(!isFinite(e)||e<-90||e>90)throw new M(C.INVALID_ARGUMENT,"Latitude must be a number between -90 and 90, but was: "+e);if(!isFinite(t)||t<-180||t>180)throw new M(C.INVALID_ARGUMENT,"Longitude must be a number between -180 and 180, but was: "+t);this._lat=e,this._long=t}get latitude(){return this._lat}get longitude(){return this._long}isEqual(e){return this._lat===e._lat&&this._long===e._long}_compareTo(e){return H(this._lat,e._lat)||H(this._long,e._long)}toJSON(){return{latitude:this._lat,longitude:this._long,type:$e._jsonSchemaVersion}}static fromJSON(e){if(gr(e,$e._jsonSchema))return new $e(e.latitude,e.longitude)}}$e._jsonSchemaVersion="firestore/geoPoint/1.0",$e._jsonSchema={type:oe("string",$e._jsonSchemaVersion),latitude:oe("number"),longitude:oe("number")};/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Oe{constructor(e){this._values=(e||[]).map((t=>t))}toArray(){return this._values.map((e=>e))}isEqual(e){return(function(r,s){if(r.length!==s.length)return!1;for(let o=0;o<r.length;++o)if(r[o]!==s[o])return!1;return!0})(this._values,e._values)}toJSON(){return{type:Oe._jsonSchemaVersion,vectorValues:this._values}}static fromJSON(e){if(gr(e,Oe._jsonSchema)){if(Array.isArray(e.vectorValues)&&e.vectorValues.every((t=>typeof t=="number")))return new Oe(e.vectorValues);throw new M(C.INVALID_ARGUMENT,"Expected 'vectorValues' field to be a number array")}}}Oe._jsonSchemaVersion="firestore/vectorValue/1.0",Oe._jsonSchema={type:oe("string",Oe._jsonSchemaVersion),vectorValues:oe("object")};/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
현재: `+location.protocol);const S=e("voice-modal");if(!S){alert("오류: 음성 모달 요소를 찾을 수 없습니다.");return}if(e("voice-transcribed-text").innerText="",e("voice-result-box").style.display="none",e("voice-status-text").innerText="마이크 초기화 중...",S.style.display="flex",n.recognition)try{console.log("Calling recognition.start()..."),n.recognition.start()}catch(P){console.error("Recognition Start Error:",P),P.name!=="InvalidStateError"&&alert("마이크 시작 오류: "+P.message)}else alert("이 브라우저나 기기에서는 음성 인식을 지원하지 않습니다.")},confirmVoiceEntry:async()=>{if(!n.lastDetected){alert("인식된 내용이 없습니다.");return}if(!n.currentUser){alert("로그인 정보가 없습니다. 다시 로그인해 주세요."),s("/");return}const S=document.querySelector("#voice-result-box .btn-primary");S&&(S.disabled=!0,S.innerText="저장 중...");try{const P={...n.lastDetected};if(e("voice-modal").style.display="none",n.lastDetected=null,S&&(S.disabled=!1,S.innerText="저장하기"),n.recognition)try{n.recognition.stop()}catch{}const b=mu(yi(jr,"users",n.currentUser.uid,"records"),P),N=new Promise((B,Q)=>setTimeout(()=>Q(new Error("Firestore Timeout (20s)")),2e4)),O=await Promise.race([b,N]);console.log("Firestore Save Success - ID:",O.id),r("심어두기가 완료되었습니다! 🎉")}catch(P){console.error("Firestore Save Error/Timeout:",P)}},cancelVoiceModal:()=>{if(console.log("cancelVoiceModal clicked"),n.recognition)try{n.recognition.stop()}catch{}n.lastDetected=null,e("voice-modal").style.display="none",e("voice-transcribed-text").innerText="",e("voice-result-box").style.display="none",e("voice-status-text").innerText="듣고 있습니다..."},clearVoiceTranscript:()=>{n.recognition&&n.recognition.start()},openAddModal:()=>e("edit-modal").style.display="flex",saveManualEntry:async S=>{if(S&&S.preventDefault(),!n.currentUser){alert("로그인 정보가 없습니다."),s("/");return}const P={date:e("edit-date").value||new Date().toISOString().split("T")[0],type:e("edit-type").value,category:e("edit-category").value,amount:parseInt(e("edit-amount").value)||0,status:"준비"};try{await mu(yi(jr,"users",n.currentUser.uid,"records"),P),alert("내역이 수동으로 저장되었습니다."),e("edit-modal").style.display="none"}catch(b){console.error("Manual Save Error:",b),alert("수동 저장 실패: "+b.message)}},deleteRecord:async S=>{confirm("정말 삭제하시겠습니까?")&&await GE(bh(jr,"users",n.currentUser.uid,"records",S))},showYearlyCategorySummary:()=>{const S={};n.records.forEach(b=>{const N=b.category||"기타필요경비",O=b.label||N,B=n.categories.find(G=>G.id===N)||{},Q=B.box?` [${B.box}]`:"";S[O]||(S[O]={amount:0,box:Q}),S[O].amount+=Number(b.amount)||0});let P='<div style="font-size:0.9rem;">';for(const[b,N]of Object.entries(S))P+=`
                    ...
                    </div>`;P+="</div>",e("report-title").innerText="올해 실적 상세 리포트",e("report-content").innerHTML=P||"데이터가 없습니다.",e("report-modal").style.display="flex"},showPrevYearSummary:()=>{const P=new Date().getFullYear(),b=n.records.length,N=n.records.reduce((B,Q)=>B+(Number(Q.amount)||0),0);let O=`
